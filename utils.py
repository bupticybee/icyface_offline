...
class Dataset():
    ...
    def next_batch(self,batch_size,shuffle = True):
        ...
        if start == 0 and self._epochs_completed == 0:
            self.idx = np.arange(0, self._num_examples)  # get all possible indexes
            np.random.shuffle(self.idx)  # shuffle indexe
        ...
        if start + batch_size > self._num_examples:
            ...
            data_rest_part = self.data[self.idx[start:self._num_examples]]
            label_rest_part = self.label[start:self._num_examples]
            self.idx = np.arange(0, self._num_examples)  # get all possible indexes
            np.random.shuffle(self.idx0)  # shuffle indexes
            ...
            data_new_part =  self._data[self.idx[start:end]]  
            label_new_part = self._label[self.idx[start:end]]
            return np.concatenate((data_rest_part, data_new_part), axis=0),np.concatenate((label_rest_part, label_new_part), axis=0)
        else:
            self._index_in_epoch += batch_size
            end = self._index_in_epoch
            return self._data[self.idx[start:end]],self._label[self.idx[start:end]]
